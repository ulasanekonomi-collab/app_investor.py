...
investment = st.sidebar.number_input("Total Investasi (Rp)", value=120_000_000)
unit_price = st.sidebar.number_input("Harga 1 Unit Vila (Rp)", value=1_200_000_000)

occupancy = st.sidebar.slider("Occupancy (%)", 30, 90, 60)
price = st.sidebar.number_input("Harga per Malam (Rp)", value=1_200_000)
days = st.sidebar.number_input("Hari Operasional", value=365)

share = st.sidebar.slider("Share Investor (%)", 10, 100, 60)
cost_ratio = st.sidebar.slider("Biaya Operasional (%)", 0, 80, 30) / 100.0

# ========================
# CALCULATION
# ========================
ownership = investment / unit_price if unit_price > 0 else 0.0

revenue = occupancy / 100.0 * price * days
income = revenue * (1 - cost_ratio) * (share / 100.0) * ownership
roi = (income / investment) * 100.0 if investment > 0 else 0.0
payback = investment / income if income > 0 else 0.0
...
def create_sensitivity_chart(price, days, cost_ratio, share, ownership, investment):
    ...
    for occ in occ_range:
        rev = occ / 100.0 * price * days
        inc = rev * (1 - cost_ratio) * (share / 100.0) * ownership
        r = (inc / investment) * 100.0 if investment > 0 else 0.0
        roi_list.append(r)
...
# ========================
narasi = f"""
Berdasarkan hasil simulasi, investasi sebesar Rp {investment:,.0f} menghasilkan pendapatan tahunan sekitar Rp {income:,.0f} 
dengan tingkat pengembalian investasi (ROI) sebesar {roi:.2f}%.

Dengan asumsi tingkat okupansi {occupancy}%, biaya operasional {cost_ratio*100:.0f}%, dan skema bagi hasil {share}%, 
investasi ini memiliki periode pengembalian modal sekitar {payback:.1f} tahun.

Distribusi simulasi menunjukkan bahwa ROI cenderung berada di kisaran {np.mean(roi_sim):.2f}% 
dengan variasi moderat, sehingga investasi ini dapat dikategorikan sebagai instrumen dengan risiko menengah.
"""
